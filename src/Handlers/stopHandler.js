...
const StopRequestFunction= async function(){
	try {
		await dbController.getConstant('stopOutput')
			.then(result => {
		...
		console.log("error en StopRequestFunction function " + error);
		return Promise.reject(_response.errorMessage = constants.errorMessage);
	}
}
...
module.exports.StopRequestFunction = StopRequestFunction;	